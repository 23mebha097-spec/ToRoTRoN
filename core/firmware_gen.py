def generate_esp32_firmware(robot, default_speed=50, motor_assignments=None):
    ...
    c.append("  Serial.println(\"BOOT:ESP32-S3\");")
    c.append("")
    ...
    c.append("    char c = (char)Serial.read();")
    c.append("    if (c == '\\r') continue;")
    c.append("    if (c == '\\n') {")
    c.append("      serialLine[serialLinePos] = '\\0';")
    c.append("      String cmd = String(serialLine);")
    c.append("      cmd.trim();")
    c.append("      serialLinePos = 0;")
    c.append("      if (cmd.length() == 0) continue;")
    c.append("      if (cmd == \"?\" || cmd == \"PING\") { Serial.println(\"PONG\"); continue; }")
    c.append("      if (cmd == \"HOME\") { goHome(); continue; }")
    c.append("      parseCommand(cmd);")
    c.append("      continue;")
    c.append("    }")
    c.append("")
    c.append("    if (serialLinePos < (SERIAL_LINE_BUF - 1)) {")
    c.append("      serialLine[serialLinePos++] = c;")
    c.append("    } else {")
    c.append("      serialLinePos = 0;")
    c.append("      Serial.println(\"ERR: RX overflow\");")
    c.append("    }")
    c.append("  }")
    c.append("}")
    c.append("")
    ...
    if need_servo_lib:
        ...
        c.append("      Serial.print(\"ACK:\"); Serial.print(sJoints[i].name);")
        c.append("      Serial.print(\":SERVO:\"); Serial.print(angle, 2);")
        c.append("      Serial.print(\":\"); Serial.println(sJoints[i].speed_pct, 2);")
        c.append("      return;")
        c.append("    }")
        c.append("  }")
        c.append("")
    ...
    if need_stepper_lib:
        ...
        c.append("      Serial.print(\"ACK:\"); Serial.print(stJoints[i].name);")
        c.append("      Serial.print(\":STEP:\"); Serial.print(angle, 2);")
        c.append("      Serial.print(\":\"); Serial.println(stJoints[i].speed_pct, 2);")
        c.append("      return;")
        c.append("    }")
        c.append("  }")
        c.append("")
    ...
    if need_servo_lib:
        ...
        c.append("    } else {")
        c.append("      // Speed_pct ∈ [0,100] → step ∈ [0.05, 4.5] deg per control tick")
        c.append("      float step = (sJoints[i].speed_pct / 100.0f) * 4.5f;")
