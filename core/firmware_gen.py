def generate_esp32_firmware(robot, default_speed=50, motor_assignments=None):
    ...
    if need_servo:
        ...
        c.append("  float  integral;")
        c.append("  float  last_error;")
        c.append("};")
    ...
    for i, (name, jobj, pwm, mode) in enumerate(servo_joints):
        ...
        c.append(f"  sJoints[{i}].integral = 0.0f;")
        c.append(f"  sJoints[{i}].last_error = 0.0f;")
        if pwm != -1:
    ...
    c.append("    return;")
    c.append("  }")
    c.append("  if (cmd.startsWith(\"PID:\")) {")
    c.append("    use_pid = (cmd.substring(4).toInt() == 1);")
    c.append("    for (int i=0; i<" + str(num_servo) + "; i++) { sJoints[i].integral = 0; sJoints[i].last_error = 0; }")
    c.append("    Serial.print(\"💡 [HW] PID Control set to: \");")
    c.append("    Serial.println(use_pid ? \"ON\" : \"OFF\");")
    c.append("    return;")
    c.append("  }")
    ...
    if need_servo:
        ...
        c.append("    } else {")
        c.append("      // Standard: interpolate or PID then move")
        c.append("      float err = sJoints[i].target_deg - sJoints[i].current_deg;")
        c.append("      if (use_pid) {")
        c.append("        float dt = CONTROL_PERIOD_MS / 1000.0f;")
        c.append("        sJoints[i].integral += err * dt;")
        c.append("        float deriv = (err - sJoints[i].last_error) / dt;")
        c.append("        sJoints[i].last_error = err;")
        c.append("        // Smooth generic PID parameters for hobby servos")
        c.append("        float P = 1.8f; float I = 0.2f; float D = 0.05f;")
        c.append("        float output = (P * err) + (I * sJoints[i].integral) + (D * deriv);")
        c.append("        float max_step = (sJoints[i].speed_pct / 100.0f) * 10.0f;")
        c.append("        output = constrain(output, -max_step, max_step);")
        c.append("        sJoints[i].current_deg += output;")
        c.append("        if (fabs(err) < 0.1f) {")
        c.append("          sJoints[i].current_deg = sJoints[i].target_deg;")
        c.append("          sJoints[i].integral = 0.0f;")
        c.append("        }")
        c.append("      } else {")
